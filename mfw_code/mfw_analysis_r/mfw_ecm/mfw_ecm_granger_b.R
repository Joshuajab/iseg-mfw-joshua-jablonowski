...
model_filename <- here::here("mfw_code", "mfw_analysis_r", "mfw_ecm_output", "best_b_ecm_model.rds")
...
# 2A: Test for Long-Run Granger Causality (s --> b); H0: Coefficient of 'b_ec_term' = 0
restricted_model_lr <- update(unrestricted_model, . ~ . - b_ec_term)
...
# 2B: Test for Short-Run Granger Causality (s --> b); H0: All coefficients for delta_s_t lags are simultaneously zero.

all_coeffs <- names(coef(unrestricted_model))
# Find which of those are the delta_s_t lag terms we need to test
sr_causality_vars <- all_coeffs[grep("^delta_s_t_lag", all_coeffs)]
...
output_filename <- here::here(output_dir, "granger_b_results.rds")
saveRDS(results_b, file = output_filename)

cat("--- Granger Test (s-->b) Complete ---\n")
