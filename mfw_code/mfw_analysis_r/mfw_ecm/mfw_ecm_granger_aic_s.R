...
# 2A: Define the grid of lag combinations to test
...
for (i in 1:nrow(lag_combinations)) {
  ...
  # --- Programmatically create the formula terms based on the new mapping ---
  lags_s <- paste0("delta_s_t_lag", 1:m, collapse = " + ")
  lags_b <- paste0("delta_b_t_lag", (1:n) + 1, collapse = " + ")
  
  # --- Build the full model formula for the s_t equation ---
  model_formula <- as.formula(
    paste("s_t ~ s_t_lag1 + s_ec_term +", lags_s, "+", lags_b)
  )
  
  # Fit the linear model using the static, complete data frame
  fit <- lm(model_formula, data = df_analysis)
  
  # Store the fitted model in our list with a new descriptive name
  model_name <- paste0("s_ecm_m", m, "_n", n)
...
cat("--- AICc Model Selection Table for s_t Model ---\n")
print(aic_table)
...
# Define a new filename for this model to avoid overwriting the previous one
output_filename <- here::here("mfw_code", "mfw_analysis_r", "mfw_ecm_output", "best_s_ecm_model.rds")
saveRDS(best_model, file = output_filename)

cat("\n--- Best Fitting Model (s_t Equation) ---\n")
